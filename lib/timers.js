...
const NodeImmediate = process.binding('immediate_wrap').Immediate;
...
function insert(item, msecs) {
  ...
  if (true)
  {
    /* JD: Nevermind. Give each timer its own libuv Timer for a 1:1 correspondence. */
    var timer = new Timer();
    timer.start(msecs, 0);
    timer[kOnTimeout] = item._onTimeout;
    item.correspondingTimer = timer; /* So that when we cancel item, we can stop the timer beneath it. */
  }
  else
  {
    if (lists[msecs]) {
...
exports.setImmediate = function(callback, arg1, arg2, arg3) {
  ...
  /* JD: Implement as an individual 'NodeImmediate'. */
  if (true) {
    var imm = new NodeImmediate();
    imm.oncheck = function () {
      immediate._onImmediate();
    };
    imm.start();
    ret = imm;
